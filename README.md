...
- **Peak hold** — running max per bin, with its own reset and an always-visible "Enable" checkbox in its own fieldset (also toggleable from the plot legend once there's data); auto-resets (with a notice) if the point count changes.
...
- **Horizontal helper lines** — click the Y-axis label area to drop a dashed threshold line at that level (with an inline label). Drag an existing one to reposition it (cursor turns to a resize handle, with a hover tooltip); right-click it to delete. Persists across reloads. Works while noise-floor subtraction is active too — a line placed then is stored as a plain relative dB delta rather than an absolute level.
- **Reference/limit lines** — built-in FCC Part 15 and CISPR 32 Class A/B radiated templates (3m only) plus an FCC Part 15 Class B conducted (AC mains, via LISN) template (see disclaimer below), sorted alphabetically, plus a custom breakpoint editor with an Edit button (Remove is no longer the only way to change one). Custom lines have their own enable/disable checkbox, same as the built-in templates, so you can hide one temporarily without deleting it. The editor's default level adapts to the current Y-axis unit, so a freshly-added line starts inside a typical visible range instead of off-chart.
...
The built-in FCC/CISPR presets are **illustrative templates** — their numeric breakpoints were checked against independent published sources, but they are not a substitute for the current official standard text; verify against the real spec before relying on them for an actual compliance decision. The radiated ones are expressed in dBµV/m (a field-strength convention); to compare them meaningfully against this app's dBm readings, set the **calibration offset** (antenna factor + cable loss) and switch the Y-axis unit to dBµV. The conducted preset is plain dBµV (a direct LISN voltage reading) — no antenna factor applies, just cable loss if any. Only 3m radiated distances are offered: CISPR 32 Class B and FCC Part 15 Class B both publish their own 3m table directly, but neither standard publishes a 3m Class A table (Class A is inherently a 10m-measurement class) — those two are a computed +10dB offset from the documented 10m table instead, flagged as such in code and in each line's tooltip.
